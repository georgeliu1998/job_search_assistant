...
page = st.sidebar.radio(
    "Select a page",
    ["Home", "Job Evaluation", "Resume Customization", "Settings"]
)
...
if page == "Home":
    ...
    with col1:
        ...
        st.markdown("Analyze job descriptions to find the best matches for your skills and preferences.")
        st.button("Try Job Evaluation", key="job_eval_btn")
    
    with col2:
        st.info("### Resume Customization")
        st.markdown("Automatically tailor your resume to highlight relevant skills for specific job opportunities.")
    ...
    if st.button("Evaluate Job"):
        if job_description:
            ...
            st.info("Job evaluation functionality will be implemented in a future update.")
    ...
    if st.button("Customize Resume"):
        if job_description:
            # This would connect to the backend in the real implementation
            st.info("Resume customization functionality will be implemented in a future update.")
...
# Footer
st.markdown("---")
st.markdown("© 2025 Job Search Assistant | [GitHub](https://github.com/georgeliu1998/job_search_assistant)") 